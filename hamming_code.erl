...
set_check_bit(Pos, CheckArray, ExtendedNum) ->
	...
	if Bit =:= 1 ->
		...
		BitNum = (Pos-1) rem 8,
		BitShift = 7-BitNum,
		{1, list_to_bitstring(cond_map(fun change_bit/2,BitShift,bitstring_to_list(ExtendedNum),SecNum))};
	true -> {0, ExtendedNum}
	end.
...
decode_num({BitList, ExtendedNum}) ->
	PosArray = lists:zip([1,2,4,8,16,32], BitList),
	PosArrayWithExtendedNum = lists:map(fun({Pos, Bit}) -> {Pos, Bit, ExtendedNum} end, PosArray),
	NewPosArray = lists:takewhile(fun check_bit_at/1,PosArrayWithExtendedNum),
	if length(NewPosArray) < length(BitList) ->
		(length(NewPosArray)+1); % if original control bits list and <BitList> don't coincide -> output number of broken bit.
		true -> get_num_from_extended(ExtendedNum)
	end.